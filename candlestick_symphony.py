...
def search_doji():
    ...
    time_m_tmp = '-15'
    print()
    print('System  time:', time.strftime('%Y-%m-%d %H:%M', time.localtime()), 'Start the bot in search of the doji star...')
    while flag == True:
        time.sleep(1)
                ...
                if time_m_tmp == '-15':
                    ...
                    if Open <= Close and Close < Open + doji_difference or Open >= Close and Close > Open - doji_difference:
                        ...
                        if Open2 < Close2 and Open3 < Close3 and High > High2 and High2 > High3:
                            print()
                            print(bcolors.FAIL + 'System  time:', time.strftime('%Y-%m-%d %H:%M', time.localtime()), 'Short signal' + bcolors.ENDC)
                            if flag_short == False:
                                flag_short = True
                                count_main -= 1
                            ...
                            print()
                            print(bcolors.OKGREEN + 'System  time:', time.strftime('%Y-%m-%d %H:%M', time.localtime()), 'Long signal' + bcolors.ENDC)
                            if flag_long == False:
                                flag_long = True
                                count_main -= 1
                            ...
                            count_main -= 1
                            break
                        print('System  time:', time.strftime('%Y-%m-%d %H:%M', time.localtime()), 'Doji appeared in the wrong conditions, we are waiting for the next one ...')
                        time_m_tmp = time_m
                        Open, Close, High, Low = 0, 0, 0, 0
                        continue
                    else:
                        print('System  time:', time.strftime('%Y-%m-%d %H:%M', time.localtime()), 'Open:', Open, 'Close:', Close, 'Candle not recognized')
                        time_m_tmp = time_m
                        Open, Close, High, Low = 0, 0, 0, 0
        else:
            time_m_tmp = time_m
